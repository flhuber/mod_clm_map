...
class ModCLMMapHelper
{
    ...
    const tab10 = [[0.1216, 0.4667, 0.7059], [1.0, 0.498, 0.0549], [0.1725, 0.6275, 0.1725], [0.8392, 0.1529, 0.1569], [0.5804, 0.4039, 0.7412], [0.549, 0.3373, 0.2941], [0.8902, 0.4667, 0.7608], [0.498, 0.498, 0.498], [0.7373, 0.7412, 0.1333], [0.0902, 0.7451, 0.8118]];
    ...
    public static function getTeamList($selectedID, $params){
        ...
        $query = "SELECT m.id, m.sid, m.zps, m.name, m.lokal_coord, m.liga, m.tln_nr, l.name AS liga_name
                    ...
                    WHERE m.liga IN ($selectedIDString) AND m.sid = l.sid AND m.published = 1 AND l.published = 1"; 
        $db->setQuery($query);
    ...
    public static function getClubList($params){
        ...
        else
        {
            ...
            elseif($mode==2)
            {
                $selectedEntries = $params->get('grouping_region');
            }
            else
            {
...
private static function getColorArray($num){   
    ...
    {
        for ($i = 0; $i < $num; $i++){
            $colorArray[$i] = self::getValueFromDiscreteArray(self::tab10, $i/$num);
        }
    } 
    elseif ($num <= 20) //Use tab20 
    {
        for ($i = 0; $i < $num; $i++){
